...
function Get(options) {
    ...
    var uri_o = url.parse(this.uri);

    function getProxyEnv(protocol) {
        if (protocol === 'http:') {
    ...
    var proxy_env = getProxyEnv(uri_o.protocol);

    if (!this.no_proxy && proxy_env) {
        this.proxy = url.parse(proxy_env);
...
Get.prototype.request = function(callback) {
    ...
    if ((Object.keys(this.proxy).length == 0) && this.uri_o.protocol == 'https:') {
        opts.port = this.proxy.port || this.uri_o.port || 443;
        return https.request(opts, callback);
    } else {
        opts.port = this.proxy.port || this.uri_o.port || 80;
