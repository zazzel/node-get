...
Get.prototype.client = function(callback) {
    ...
    if (!(this.uri_o.protocol &&
        ...
        this.uri_o.hostname)) {
        return callback.call(this, { message: 'Invalid URL.' });
    }
...
Get.prototype.toDisk = function(filename, callback) {
    ...
    this.perform(function(err, res) {
        ...
        file.on('close', function() { callback(null, filename); });
        res.pipe(file);
    });
};
    ...
    this.perform(function(err, response) {
        ...
        switch ((response.headers['content-type'] || '').toLowerCase()) {
            ...
            case 'application/octet-stream':
                return callback(new Error("Can't download binary file"));
            default:
